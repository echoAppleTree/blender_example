...
class hops(PropertyGroup):
    ...
    menu_array_type: EnumProperty(
        name="Array Q Menu Option Type",
    ...
    bool_scroll_expanded: EnumProperty(
        name="Default Launch UI",
        description="Which UI System to default to",
        items=[
            ('FAST', "Fast", "Standard modal UI"),
            ('EXPANDED', "Expanded", "Expanded modal UI")],
        default='FAST')
...
def draw(preference, context, layout):
    ...
    if preference.property.show_hardops_properties:
        ...
        sub_box = sub_box.box()
        label_row(preference.property, 'ko_popup_type', sub_box.row(), label='KitOps Popup Type')
        label_row(preference.property, 'menu_array_type', sub_box.row(), label='Array')
        if get_preferences().property.menu_array_type == 'ST3':
            label_row(preference.property, 'array_type', sub_box.row(), label='Array V1 Gizmo')
        ...
        if get_preferences().property.bool_scroll_type != 'CLASSIC':
            label_row(preference.property, 'bool_scroll_expanded', sub_box.row(), label='Scroll UI')
        ...
        # Clean
        sub_box = split.box()
        sub_box.label(text="Mesh Clean")
        sub_box = sub_box.box()
        label_row(preference.property, 'meshclean_mode', sub_box.row(), label='Behavior')
        ...
        sub_box = sub_box.box()
        label_row(preference.behavior, 'mat_viewport', sub_box.row(), label='Blank:View Material')
        label_row(preference.property, 'Hops_twist_radial_sort', sub_box.row(), label='Radial / Twist Render')
        label_row(preference.property, 'to_cam', sub_box.row(), label='To_Cam Behavior')
        ...
        label_row(preference.property, 'multi_tool_entry', sub_box.row(), label='Multi Tool Entry')
        label_row(preference.property, 'array_v2_use_2d', sub_box.row(), label='Array V2 Method')

        # Lights
        sub_box = split.box()
        sub_box.label(text="Lights")
        sub_box = sub_box.box()
