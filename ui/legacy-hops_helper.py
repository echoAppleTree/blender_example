...
class HOPS_OT_HelperPopup(bpy.types.Operator):
    ...
    def draw_misc_tab(self, context, layout):
        ...
        if option.operators_options:
            ...
            colrow = col.row(align=True)
            colrow.prop(get_preferences().property, "Hops_mirror_modal_scale", text="Mirror Scale")
            colrow.prop(get_preferences().property, "Hops_mirror_modal_sides_scale", text="Mirror Size")
            colrow = col.row(align=True)
