...
class Scroll_Widget():
    ...
    def __init__(self, db, show_bar=True):
        ...
        # Scroll Bar
        self.hide_scroll_bar_override = not show_bar
        self.scroll_bar = Scroll_Bar(db=self.db, show_bar=show_bar)
        self.bar_width = 0
        self.body_height = 0
    ...
    def body_setup(self):
        ...
        if self.hide_scroll_bar_override:
            self.layout_copy.setup(
                bottom_left  =(self.bottom_left[0], self.bottom_left[1]), 
                bottom_right =(self.bottom_right[0], self.bottom_right[1]),
                top_left     =(self.top_left[0], self.top_left[1] - self.header_height),
                top_right    =(self.top_right[0], self.top_right[1] - self.header_height))
        else:
            self.layout_copy.setup(
                bottom_left  =(self.bottom_left[0], self.bottom_left[1]), 
    ...
    def draw(self):
        ...
        if not self.header_controller.collapsed:
            ...
            # Scroll Bar
            if not self.hide_scroll_bar_override:
                if self.show_scroll_bar:
...
class Scroll_Bar():
    ...
    def event(self):
        ...
        if self.dragging:
            ...
            self.mouse_is_over = mouse_over_scroll_box
    ...
    def draw(self):
        ...
        if self.show_bar == False:
            if self.dragging or self.mouse_is_over:
                self.bar.draw()
