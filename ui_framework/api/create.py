...
class Create():
    ...
    def widget_scroll(self, panel=None, win_key="", collapsable=False, split_count_override=False, split_count=1, show_bar=True):
        '''Insert a scroll widget into the panel.'''

        widget = Scroll_Widget(db=self.db, show_bar=show_bar)
        widget.win_key = win_key
    ...
    def element_image(self, layout=None, image=None, scale=1, force_fit=False, padding=6, maximize=False, max_ratio_size=False):
        '''Add the element to the lastly created cell on the layout'''
        ...
        element.max_ratio_size = max_ratio_size
    ...
    def event_call_back(self, layout=None, func=None, positive_args=None, negative_args=None, scrollable=False, shift_func=None, shift_arges=None):
        ...
        event.shift_func = shift_func
        event.shift_arges = shift_arges
