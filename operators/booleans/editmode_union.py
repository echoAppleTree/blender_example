...
def edit_bool_union(context, keep_cutters, use_swap, use_self, threshold, solver):
    def select(geom):
        for g in geom:
            g.select = True
    ...
    if bpy.app.version[1] > 83:
        bpy.ops.mesh.intersect_boolean(operation='UNION', use_swap=use_swap, use_self=use_self, threshold=threshold, solver=solver)
    else:
        bpy.ops.mesh.intersect_boolean(operation='UNION', use_swap=use_swap, threshold=threshold)
...
LMB + Ctrl - Keep cutters after use"""
    ...
    def execute(self, context):
        ...
        return edit_bool_union(context, self.keep_cutters, self.use_swap, self.use_self, self.threshold, get_preferences().property.boolean_solver)
