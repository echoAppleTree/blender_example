...
class DecapCaps:
    ...
    def __init__(self, context, relative_name=''):
        ...
        self.mesh_a.use_auto_smooth = True
        ...
        self.mesh_b.use_auto_smooth = True
...
class HOPS_OT_To_Shape_V2(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        # --- Batches --- #
        self.edges_shader = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
        self.edges_batch = None

        self.decap_faces_shader = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
        self.decap_faces_batch = None

        self.decap_keep_caps_shader = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
        self.decap_keep_caps_batch = None

        self.shape_edges_shader = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
        self.shape_edges_batch = None

        self.shape_faces_shader = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
    ...
    def confirmed_exit(self, context):
        ...
        # Using decap so remove other shapes
            ...
            self.decap_obj.data.use_auto_smooth = True
            bpy.ops.hops.draw_wire_mesh_launcher('INVOKE_DEFAULT', object_name=self.decap_obj.name)
