...
class HOPS_OT_ModalMirror(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        return getattr(active_object, "type", "") in {"MESH", "EMPTY", "CURVE", "GPENCIL"}
            ...
            if event.value == 'PRESS':
                if self.multiselection:
                    if inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad4):
                        get_preferences().property.Hops_mirror_modes_multi = "VIA_ACTIVE"

                    elif inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad3):
                        get_preferences().property.Hops_mirror_modes_multi = "SYMMETRY"

                else:
                    if inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad2):
                        get_preferences().property.Hops_mirror_modes = "SYMMETRY"

                    elif inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad3):
                        get_preferences().property.Hops_mirror_modes = "BISECT"

                    elif inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad4):
                        get_preferences().property.Hops_mirror_modes = "MODIFIER"

                    elif inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad5):
                        get_preferences().property.Hops_mirror_modal_use_cursor = True

                    elif inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.quad6):
                        get_preferences().property.Hops_mirror_modal_use_cursor = False

                if inside_polygon(self.mouse_pos[0], self.mouse_pos[1], self.points_x_p_region):
                    self._operation = "MIRROR_X"
                    if get_preferences().property.Hops_mirror_modal_revert:
                        get_preferences().property.Hops_mirror_direction = "-"
                    ...
                    if get_preferences().property.Hops_mirror_modal_use_cursor:
    ...
    def draw(self, context):
        method_handler(self._draw,
            arguments = (context, ),
            identifier = 'Modal Mirror UI Shader',
    ...
    def _draw(self, context):
        ...
        scale_value = get_preferences().property.Hops_mirror_modal_sides_scale
        zoom_fix = rv3d.view_distance / bpy.context.space_data.lens
        zoom_value = get_preferences().property.Hops_mirror_modal_scale
        ...
        if get_preferences().property.Hops_mirror_modal_use_cursor:
            if self.multiselection:
                ...
                center_loc = bpy.context.scene.cursor.location
        else:
            center_loc = object.location
        ...
        mirror_interface_scale = get_preferences().property.Hops_mirror_modal_Interface_scale
        ...
        if self.multiselection is False:
            ...
            glEnable(GL_BLEND)
            if get_preferences().property.Hops_mirror_modes == "SYMMETRY":
            ...
            glEnable(GL_BLEND)
            if get_preferences().property.Hops_mirror_modes == "BISECT":
            ...
            glEnable(GL_BLEND)
            if get_preferences().property.Hops_mirror_modal_use_cursor is False:
            ...
            glEnable(GL_BLEND)
            if get_preferences().property.Hops_mirror_modes_multi == "SYMMETRY":
                draw_text("Symmetry", self.quad4[0][0]+4, self.quad4[0][1]-14, align="LEFT", size=10, color=(1, 1, 1, 1))
                glColor4f(0.35, 1, 0.29, 0.53)
            else:
                glColor4f(0.1, 0.1, 0.1, 0.6)
