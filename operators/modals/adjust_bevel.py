    ...
    def modal(self, context, event):
        ...
        for object_name in self.bevel_objects:
            ...
            if event.type == "ONE" and event.value == "PRESS":
                ...
                if context.mode == 'EDIT_MESH':
                    ...
                    if get_preferences().ui.Hops_extra_info:
                        bpy.ops.hops.display_notification(info=F'Autosmooth : {round(degrees(object.data.auto_smooth_angle), 4)}' )
            ...
            if event.type == "THREE" and event.value == "PRESS":
                ...
                if get_preferences().ui.Hops_extra_info:
                    bpy.ops.hops.display_notification(info= F'Conversion Bevel' )
            ...
            if event.type == "L" and event.value == "PRESS":
                ...
                if get_preferences().ui.Hops_extra_info:
                    bpy.ops.hops.display_notification(info=F'Limit Method : {modifier.limit_method}' )
            ...
            if event.type == 'P' and event.value == 'PRESS' and event.shift:
                ...
                if self.support_profile_scroll:
                   ...
                   if get_preferences().ui.Hops_extra_info:
                        bpy.ops.hops.display_notification(info=F'No Profiles Found' ) 
            ...
            if self.base_controls.confirm or event.type in ( "RET", "NUMPAD_ENTER"):
                ...
                if event.type in ("RET", "NUMPAD_ENTER") and event.shift:
                    ...
                    if context.mode == 'OBJECT':
                        ...
                        if get_preferences().ui.Hops_extra_info:
                            bpy.ops.hops.display_notification(info=f'{modifier.name} : Applied' )
    ...
    def draw_ui(self, context):
        ...
        if self.master.should_build_fast_ui():
            ...
            if get_preferences().ui.Hops_modal_fast_ui_loc_options != 1:
                ...
                win_list.append("Width : "    + "%.4f" % self.active_bevel_modifier.width)
                if self.support_profile_scroll and self.scrolling_profiles:
            ...
            help_items["STANDARD"] = [
                ...
                ("L",              f"Change limit method - {self.active_bevel_modifier.limit_method}"),
                ("X",              f"Set bevel to 50% of current (half-bevel)"),
    ...
    def restore(self):
        ...
        for object_name in self.bevel_objects:
            ...
            if "added_modifier" in self.bevel_objects[object_name]:
                ...
                if bpy.app.version < (2, 90, 0):
                    if modifier.use_custom_profile is True:
                        if self.support_profile_scroll:
