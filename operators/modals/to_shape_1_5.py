...
#general
    ...
    equalize_mode: bpy.props.EnumProperty(
        ...
        items=[
            ('OFF', "OFF", "OFF"),
            ('ALL', "ALL", "Parent shape to source"),
            ('RADIUS', "RADIUS", "Parent soure to shape"),
            ],
        default='OFF')
...
#selector
    ...
    def invoke (self, context, event):
        ...
        if self.primitive_type in {'Convex_Hull','Decap'} :
            self.get_coords = self.get_coords_obj
    ...
    def modal(self, context, event):
        ...
        if self.base_controls.pass_through:
        ...
        elif event.type in {"TAB", "SPACE"} and event.value == 'PRESS':
            ...
            del self.object_params_select
            del self.object_params_active
            del self.created_shapes
            del self.hide_shapes
            del self.enable_mods
            del self.deletable
            ...
            if self.primitive_type == 'Cylinder':
                self.cyl_segments = annotations['cyl_segments'][1]['default']
                self.cyl_diameter1 = annotations['cyl_diameter1'][1]['default']
                self.cyl_diameter2 = annotations['cyl_diameter2'][1]['default']

            elif self.primitive_type == 'Sphere':
                self.sphere_segments = annotations['sphere_segments'][1]['default']
                self.sphere_rings = annotations['sphere_rings'][1]['default']

            elif self.primitive_type == 'Convex_Hull':
                self.dissolve_angle = annotations['dissolve_angle'][1]['default']

            elif self.primitive_type == 'Decap':
                self.decap_thickness = annotations['decap_thickness'][1]['default']
                self.decap_center = annotations['decap_center'][1]['default']
                self.decap_solidify = annotations['decap_solidify'][1]['default']
                self.decap_solidify_multi = annotations['decap_solidify_multi'][1]['default']
    ...
    def create_shape(self, context, flair = True):
        ...
        if self.individual:
            ...
            for obj_data in object_params:
                obj = bpy.data.objects[obj_data.object_name]
                bounds = obj_data.bounds_final if self.modified else obj_data.bounds_base
    ...
    def enum_scroll(self, prop, value = 1):
        items = [item[0] for item in self.__annotations__[prop][1]['items']]
    ...
    def draw_ui(self, context):
        ...
        if self.master.should_build_fast_ui():
            ...
            if self.mouse_state == Mouse_states.hull_dissolve:
                ...
                help_append(["Ctrl+Mouse ", "Adjust Diameter 2"])
                help_append(["Mouse", "Adjust Diameter 1"])
...
def popup_draw(self, context):
    ...
    vals = (item[0] for item in data.__annotations__['primitive_type'][1]['items'])
