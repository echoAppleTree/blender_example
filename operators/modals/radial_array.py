...
Shift + LMB - Array selected object around active object"""
    ...
    def invoke(self, context, event):
        ...
        if event.shift and not event.ctrl and len(self.selected_states) == 1:
            ...
            self.selection_location = context.active_object.matrix_world.translation
            self.obj_matrix = self.obj.matrix_world.copy()
            #copy rotation of active object to selected object
            loc, _, sca = self.obj.matrix_world.decompose()
            loc_matrix = Matrix.Translation(loc)
            rot_matrix = context.active_object.matrix_world.to_quaternion().to_matrix().to_4x4()
            sca_matrix = Matrix.Diagonal(sca).to_4x4()
            self.obj.matrix_world = loc_matrix @ rot_matrix @ sca_matrix
        ...
        self.cursor = event.ctrl and not event.shift and event.type == 'LEFTMOUSE'
    ...
    def set_origin(self, context):
        ...
        elif self.to_selection:
            self.set_origin_vector(self.selection_location)
        else:
    ...
    def set_origin_vector(self, vector_world = Vector()):
        local_vector =self.obj.matrix_world.inverted() @ vector_world
        self.obj.data.transform( Matrix.Translation(-local_vector))
    ...
    def cancel(self, context):
        ...
        self.empty.parent = None
        self.set_origin_vector(self.obj_matrix.translation)
        self.obj.matrix_world = self.obj_matrix
        self.empty.parent = self.obj
        self.empty.location = (0,0,0)
        self.empty.rotation_euler = (0,0,0)
        self.empty.scale = (1,1,1)
