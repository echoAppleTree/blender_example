...
class HOPS_OT_Sculpt_Primitives(bpy.types.Operator):
    ...
    def graphics_setup(self):
        ...
        self.center_shader_lines = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
        self.center_batch_lines = None

        self.center_indices_tris = []
        self.center_shader_tris = gpu.shader.from_builtin('3D_UNIFORM_COLOR')
    ...
    def mod_controller(self):
        ...
        if self.centering:
            self.sphere_mod.use_axis[0] = False
            self.cube_mod.use_axis[0] = False
            return

        if self.shape == 'SPHERE':
            # Right Side
            if self.sphereOBJ.location[0] > self.avg_center[0]:
                self.sphere_mod.use_axis[0] = True
                self.sphere_mod.use_bisect_flip_axis[0] = False
            # Left Side
            if self.sphereOBJ.location[0] < self.avg_center[0]:
                self.sphere_mod.use_axis[0] = True
                self.sphere_mod.use_bisect_flip_axis[0] = True

        elif self.shape == 'CUBE':
            # Right Side
            if self.cubeOBJ.location[0] > self.avg_center[0]:
                self.cube_mod.use_axis[0] = True
                self.cube_mod.use_bisect_flip_axis[0] = False
            # Left Side
            if self.cubeOBJ.location[0] < self.avg_center[0]:
                self.cube_mod.use_axis[0] = True
                self.cube_mod.use_bisect_flip_axis[0] = True
    ...
    def caster_confirm(self, context):
        ...
        if self.shape == 'SPHERE':
            ...
            bpy.ops.object.modifier_apply(modifier=self.sphere_mod.name)

        elif self.shape == 'CUBE':
            bpy.data.objects.remove(self.sphereOBJ)
            self.cubeOBJ.select_set(True)
            context.view_layer.objects.active = self.cubeOBJ
            bpy.ops.object.modifier_apply(modifier=self.cube_mod.name)
